...
/// This class computes for every Vtc apply operand
...
public:
  AccessExtents(Operation *op) {
    // Walk all apply ops of the Vtc program
    op->walk([&](Vtc::ApplyOp applyOp) {
      ...
      applyOp.walk([&](mlir::Vtc::ExtentInterface extentOp) {
        Index lb, ub;
...
protected:
  // Shape extension
  void updateStorageShape(ShapeInterface shapeOp, ArrayRef<int64_t> lb,
                          ArrayRef<std::int64_t> ub);
  bool isShapeExtension(ShapeInterface current, ArrayRef<int64_t> lb,
                        ArrayRef<std::int64_t> ub);
  std::tuple<Index, Index> getResultShape(Operation *definingOp, Value result);
  LogicalResult extendStorageShape(ShapeInterface shapeOp, Value temp);

  // Shape inference
  void updateShape(ShapeInterface shapeOp, ArrayRef<int64_t> lb, ArrayRef<int64_t> ub);
  LogicalResult updateBounds(OpOperand &use, const AccessExtents &extents,
                             Index &lower, Index &upper);
  LogicalResult inferShapes(ShapeInterface shapeOp, const AccessExtents &extents);
};

/// Update the shape given updated bounds
void ShapeInferencePass::updateShape(ShapeInterface shapeOp, ArrayRef<int64_t> lb,
                                     ArrayRef<int64_t> ub) {
  shapeOp.updateShape(lb, ub);
  // Update the region arguments of dependent shape operations
  // (needed for operations such as the Vtc apply op)
...
LogicalResult ShapeInferencePass::updateBounds(OpOperand &use,
  ...
  if (auto shapeOp = dyn_cast<ShapeInterface>(use.getOwner())) {
    if (shapeOp.hasShape()) {
      ...
      if (auto combineOp = dyn_cast<Vtc::CombineOp>(use.getOwner())) {
...
LogicalResult ShapeInferencePass::inferShapes(ShapeInterface shapeOp,
                                              const AccessExtents &extents) {
  Index lb, ub;
...
ShapeInferencePass::getResultShape(Operation *definingOp, Value result) {
  ...
  if (auto applyOp = dyn_cast<Vtc::ApplyOp>(definingOp)) {
    auto shapeOp = cast<ShapeInterface>(applyOp.getOperation());
    if (shapeOp.hasShape()) {
  ...
  if (auto combineOp = dyn_cast<Vtc::CombineOp>(definingOp)) {
...
void ShapeInferencePass::updateStorageShape(ShapeInterface shapeOp,
                                            ArrayRef<int64_t> lb,
                                            ArrayRef<std::int64_t> ub) {
...
bool ShapeInferencePass::isShapeExtension(ShapeInterface current, ArrayRef<int64_t> lb,
                                          ArrayRef<std::int64_t> ub) {
...
LogicalResult ShapeInferencePass::extendStorageShape(ShapeInterface shapeOp,
                                                     Value temp) {
  Index lb, ub;
...
void ShapeInferencePass::runOnFunction() {
  ...
  if (!Vtc::VtcDialect::isVtcProgram(funcOp))
    return;
  ...
  if (extendStorage) {
    ...
    funcOp.walk([&](Vtc::StoreOp storeOp) {
    ...
    funcOp.walk([&](Vtc::BufferOp bufferOp) {
