  ...
  for (auto op = entryBlock.rbegin(); op != entryBlock.rend(); ++op) {
    ...
    if (auto storeOp = dyn_cast<Vtc::StoreOp>(*op)) {
      negative[storeOp.getOperation()] =
          cast<ShapeInterface>(storeOp.getOperation()).getLB();
      positive[storeOp.getOperation()] =
          cast<ShapeInterface>(storeOp.getOperation()).getUB();
      // For other ShapeInterfaces split them if they participate in multiple domains
    }
    if (dyn_cast<ApplyOp>(*op) || dyn_cast<CombineOp>(*op) ||
        dyn_cast<BufferOp>(*op) || dyn_cast<Vtc::LoadOp>(*op)) {
...
void splitOnLastCombines(FuncOp funcOp) {
  // Map to remember the closest downstream Vtc combine for every operation
...
void DomainSplitPass::runOnFunction() {
  ...
  funcOp.walk([&](ShapeInterface shapeOp) {
    if (!shapeOp.hasShape())
