...
protected:
  bool areRangesOverlapping(ShapeInterface shapeOp1, ShapeInterface shapeOp2);
...
bool ShapeOverlapPass::areRangesOverlapping(ShapeInterface shapeOp1,
                                            ShapeInterface shapeOp2) {
...
SmallVector<Value, 10> ShapeOverlapPass::computeGroupValues(
  ...
  for (auto en : llvm::enumerate(group)) {
    ...
    auto shapeOp = cast<ShapeInterface>(storeOp.getOperation());
...
SmallVector<Value, 10> ShapeOverlapPass::splitGroupDimension(
  ...
  while (!lowerBounds.empty()) {
    ...
    auto combineOp = b.create<Vtc::CombineOp>(
...
void ShapeOverlapPass::runOnFunction() {
  ...
  for (auto &group : groupList) {
    ...
    for (ShapeInterface  shapeOp : group) { //ShapeInterface
