...
struct FuseRewrite : public CombineOpPattern {
  ...
  LogicalResult fuseApplyOps(ArrayRef<Operation *> definingOps,
                             Vtc::CombineOp combineOp,
                             PatternRewriter &rewriter) const {
    // Extract the apply ops
    assert(definingOps.size() >= 2 && "expected multiple apply operations");
    auto applyOp1 = cast<Vtc::ApplyOp>(definingOps[0]);
    auto applyOp2 = cast<Vtc::ApplyOp>(definingOps[1]);

    // Check the shapes match
    auto shapeOp1 = cast<ShapeInterface>(applyOp1.getOperation());
    auto shapeOp2 = cast<ShapeInterface>(applyOp2.getOperation());
    ...
    auto returnOp1 =
        cast<Vtc::ReturnOp>(applyOp1.getBody()->getTerminator());
    auto returnOp2 =
        cast<Vtc::ReturnOp>(applyOp2.getBody()->getTerminator());
    ...
    rewriter.create<Vtc::ReturnOp>(combineOp.getLoc(), newReturnOperands,
                                       returnOp1.unroll());
...
struct EmptyStoreRewrite : public CombineOpPattern {
  ...
  // Introduce empty applies to complement the extra Vtc computations
  LogicalResult introduceEmptyStores(Vtc::CombineOp combineOp,
                                     PatternRewriter &rewriter) const {
    ...
    // Introduce a new Vtc combine operation that has no extra operands
...
// Pattern replacing Vtc.combine ops by if/else
struct IfElseRewrite : public CombineOpPattern {
  ...
  permuteReturnOpOperands(Vtc::ApplyOp applyOp,
                          OperandRange combineOpOperands,
                          Vtc::ReturnOp returnOp) const {
  ...
  LogicalResult lowerVtcCombine(Vtc::ApplyOp lowerOp,
                                    Vtc::ApplyOp upperOp,
                                    Vtc::CombineOp combineOp,
                                    PatternRewriter &rewriter) const {
    ...
    auto indexOp =
        rewriter.create<Vtc::IndexOp>(loc, combineOp.dim(), offset);
    ...
    auto lowerReturnOp =
        cast<Vtc::ReturnOp>(lowerOp.getBody()->getTerminator());
    auto upperReturnOp =
        cast<Vtc::ReturnOp>(upperOp.getBody()->getTerminator());
    // Check both apply operations have the same unroll configuration if any
    ...
    auto ifOp = rewriter.create<scf::IfOp>(loc, lowerReturnOp.getOperandTypes(),
                                           cmpOp, true);
    rewriter.create<Vtc::ReturnOp>(loc, ifOp.getResults(),
  ...
  LogicalResult matchAndRewrite(Vtc::CombineOp combineOp,
    ...
    auto lowerOp = dyn_cast<Vtc::ApplyOp>(definingLowerOps[0]);
    auto upperOp = dyn_cast<Vtc::ApplyOp>(definingUpperOps[0]);
    if (lowerOp && upperOp) {
      // Lower the combine op and its predecessors to a single apply
      return lowerVtcCombine(lowerOp, upperOp, combineOp, rewriter);
    }
    return failure();
  }
};
